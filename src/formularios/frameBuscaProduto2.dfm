object BuscaProduto2: TBuscaProduto2
  Left = 0
  Top = 0
  Width = 429
  Height = 194
  TabOrder = 0
  object edtReferencia: TEdit
    Left = 8
    Top = 72
    Width = 80
    Height = 21
    TabOrder = 3
    OnChange = edtReferenciaChange
    OnKeyDown = edtReferenciaKeyDown
  end
  object lblReferencia: TStaticText
    Left = 8
    Top = 55
    Width = 66
    Height = 17
    Caption = 'Refer'#234'ncia'
    Color = clWhite
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Pitch = fpVariable
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    TabOrder = 13
    Transparent = False
  end
  object btnBuscaProduto: TBitBtn
    Left = 92
    Top = 70
    Width = 26
    Height = 25
    Glyph.Data = {
      
      9B009B9B9B009B9B9B009B9B9B009B9B9B009B9B9B009B9B9B00}
    NumGlyphs = 2
    TabOrder = 4
    TabStop = False
    OnClick = btnBuscaProdutoClick
  end
  object lblProduto: TStaticText
    Left = 122
    Top = 56
    Width = 48
    Height = 17
    Caption = 'Produto'
    Color = clWhite
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    TabOrder = 14
    Transparent = False
  end
  object edtDescricao: TEdit
    Left = 123
    Top = 72
    Width = 297
    Height = 21
    TabStop = False
    ReadOnly = True
    TabOrder = 5
  end
  object edtRefCor: TEdit
    Left = 8
    Top = 116
    Width = 80
    Height = 21
    Enabled = False
    TabOrder = 6
    OnChange = edtRefCorChange
    OnKeyDown = edtRefCorKeyDown
  end
  object lblCor: TStaticText
    Left = 8
    Top = 99
    Width = 23
    Height = 17
    Caption = 'Cor'
    Color = clWhite
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Pitch = fpVariable
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    TabOrder = 12
    Transparent = False
  end
  object btnBuscaCor: TBitBtn
    Left = 92
    Top = 114
    Width = 26
    Height = 25
    Enabled = False
    Glyph.Data = {
      
      9B009B9B9B009B9B9B009B9B9B009B9B9B009B9B9B009B9B9B00}
    NumGlyphs = 2
    TabOrder = 7
    TabStop = False
    OnClick = btnBuscaCorClick
  end
  object edtDescricaoCor: TEdit
    Left = 123
    Top = 116
    Width = 297
    Height = 21
    TabStop = False
    Enabled = False
    ReadOnly = True
    TabOrder = 8
  end
  object edtCodTamanho: TEdit
    Left = 8
    Top = 161
    Width = 80
    Height = 21
    Enabled = False
    TabOrder = 9
    OnChange = edtCodTamanhoChange
    OnKeyDown = edtCodTamanhoKeyDown
  end
  object lblTamanho: TStaticText
    Left = 8
    Top = 144
    Width = 56
    Height = 17
    Caption = 'Tamanho'
    Color = clWhite
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Pitch = fpVariable
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    TabOrder = 15
    Transparent = False
  end
  object btnBuscaTamanho: TBitBtn
    Left = 92
    Top = 159
    Width = 26
    Height = 25
    Enabled = False
    Glyph.Data = {
      
      9B009B9B9B009B9B9B009B9B9B009B9B9B009B9B9B009B9B9B00}
    NumGlyphs = 2
    TabOrder = 10
    TabStop = False
    OnClick = btnBuscaTamanhoClick
  end
  object edtDescricaoTamanho: TEdit
    Left = 123
    Top = 161
    Width = 297
    Height = 21
    TabStop = False
    Enabled = False
    ReadOnly = True
    TabOrder = 11
  end
  object lbColecao: TStaticText
    Left = 8
    Top = 11
    Width = 50
    Height = 17
    Caption = 'Cole'#231#227'o'
    Color = clWhite
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Pitch = fpVariable
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    TabOrder = 16
    Transparent = False
  end
  object btnBuscaColecao: TBitBtn
    Left = 92
    Top = 26
    Width = 26
    Height = 25
    Glyph.Data = {
      
      9B009B9B9B009B9B9B009B9B9B009B9B9B009B9B9B009B9B9B00}
    NumGlyphs = 2
    TabOrder = 1
    TabStop = False
    OnClick = btnBuscaColecaoClick
  end
  object edtColecao: TEdit
    Left = 123
    Top = 28
    Width = 297
    Height = 21
    TabStop = False
    Enabled = False
    ReadOnly = True
    TabOrder = 2
  end
  object edtCodColecao: TCurrencyEdit
    Left = 8
    Top = 28
    Width = 79
    Height = 21
    AutoSize = False
    DisplayFormat = '0'
    TabOrder = 0
    OnChange = edtCodColecaoChange
    OnKeyDown = edtCodColecaoKeyDown
  end
end
