...
begin
    if (DataInicial > DataFinal) then
      raise TExcecaoPeriodoInvalido.Create();
...
begin
   ...
   if ( NF.Entrada_saida = 'E' ) and ( self.FCNPJ_empresa_selecionada = '0' ) and (Nf.NotaDeMaterias) then begin
     result := false;
     exit;
   end;
   ...
   result := ((self.FDataInicial = 0) and (self.FDataFinal = 0)) or ((NF.DataSaida >= self.FDataInicial) and (NF.DataSaida <= self.FDataFinal));
