...
type
   ...
   public
     procedure AdicionarBuscadorNotaFiscal       (Metodo :TMetodoDelegadoObtemCampoInteger);
     procedure AdicionarAposAtualizarLocalEntrega(Metodo :TNotifyEvent);
...
function TLocalEntregaNotaFiscal.GetCodigoNotaFiscal: Integer;
begin
   try
     result := self.FBuscadorCodigoNotaFiscal;
   except
     on E: EAccessViolation do
      raise EAccessViolation.Create('Erro em: TLocalEntregaNotaFiscal.GetCodigoNotaFiscal: Integer'+#13+
                                    'O atributo FBuscadorCodigoNotaFiscal não foi instânciado!');
   end;
end;
