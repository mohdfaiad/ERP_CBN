...
procedure TItemNotaFiscal.AdicionarBuscadorCodigoNotaFiscal(
...
begin
   ...
   self.FIcms00           := TIcms00.Create(OrigemMercadoria,
                                            AliquotaICMS,
                                            PercReducaoBC); //alterar

   if NaturezaOperacao.suspensao_icms = 'S' then
     self.FIcms00.Aliquota  := 0;

   self.FIcms00.SetMetodoDelegadoParaBuscarCodigoItem(self.GetCodigo);
   self.FIcms00.SetMetodoDelegadoParaBuscarBaseCalculo(self.GetValorLiquido);

   self.FIpiTrib          := TIpiTrib.Create(AliquotaIPI);
   self.FIpiTrib.SetMetodoDelegadoParaBuscarCodigoItem(self.GetCodigo);
   self.FIpiTrib.SetMetodoDelegadoParaBuscarBaseCalculo(self.GetValorLiquido);
   ...
   self.FPisAliq.SetMetodoDelegadoParaBuscarCodigoItem(self.GetCodigo);

   self.FPisAliq.SetMetodoDelegadoParaBuscarBaseCalculo(self.GetValorLiquido);
   ...
   self.FCofinsAliq.SetMetodoDelegadoParaBuscarCodigoItem(self.GetCodigo);
   self.FCofinsAliq.SetMetodoDelegadoParaBuscarBaseCalculo(self.GetValorLiquido);
end;
...
begin
   ...
   if (AliquotaCreditoSN <= 0) then
    raise TExcecaoParametroInvalido.Create(self.ClassName, NOME_DO_METODO, 'AliquotaCreditoSN');

   self.FIcmsSn101 := TIcmsSn101.Create(OrigemMercadoria,
                                        AliquotaCreditoSN); // tirado a aliquota pq simples nao gera credito, e a aliq de credito somente nos dados adicionais da nota "AliquotaCreditoSN"
   self.FIcmsSn101.SetMetodoDelegadoParaBuscarCodigoItem(self.GetCodigo);
   self.FIcmsSn101.SetMetodoDelegadoParaBuscarBaseCalculo(self.GetValorLiquido);

   self.FIpiNt     := TIpiNt.Create;
   self.FIpiNt.SetMetodoDelegadoParaBuscarCodigoItem(self.GetCodigo);
   self.FIpiNt.SetMetodoDelegadoParaBuscarBaseCalculo(self.GetValorLiquido);

   self.FPisNt     := TPisNt.Create;
   self.FPisNt.SetMetodoDelegadoParaBuscarCodigoItem(self.GetCodigo);
   self.FPisNt.SetMetodoDelegadoParaBuscarBaseCalculo(self.GetValorLiquido);

   self.FCofinsNt  := TCofinsNt.Create;
   self.FCofinsNt.SetMetodoDelegadoParaBuscarCodigoItem(self.GetCodigo);
   self.FCofinsNt.SetMetodoDelegadoParaBuscarBaseCalculo(self.GetValorLiquido);
end;
...
function TItemNotaFiscal.GetCodigoNotaFiscal: Integer;
begin
   try
     if (self.FCodigoNotaFiscal = 0) then
       self.FCodigoNotaFiscal := self.FBuscaCodigoNotaFiscal;
   except
     on E: EAccessViolation do
      raise EInvalidOp.Create('Erro em TItemNotaFiscal.GetCodigoNotaFiscal: Integer'+#13+
