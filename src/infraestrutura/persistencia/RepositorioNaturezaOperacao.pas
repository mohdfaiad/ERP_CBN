...
procedure TRepositorioNaturezaOperacao.SetCamposAlterados(
  Auditoria: TAuditoria; AntigoObjeto, Objeto: TObject);
var
  DAntigo, DNovo :TNaturezaOperacao;
begin
   DAntigo := (AntigoObjeto as TNaturezaOperacao);
   DNovo   := (Objeto       as TNaturezaOperacao);

   if (DAntigo.Descricao <> DNovo.Descricao) then
    Auditoria.AdicionaCampoAlterado('descricao', DAntigo.Descricao, DNovo.Descricao);

   if (DAntigo.CFOP <> DNovo.CFOP) then
    Auditoria.AdicionaCampoAlterado('cfop', DAntigo.CFOP, DNovo.CFOP);

   if (DAntigo.suspensao_icms <> DNovo.suspensao_icms) then
    Auditoria.AdicionaCampoAlterado('suspensao_icms', DAntigo.suspensao_icms, DNovo.suspensao_icms);
end;

procedure TRepositorioNaturezaOperacao.SetCamposExcluidos(
...
begin
   ...
   Auditoria.AdicionaCampoExcluido('descricao',     Obj.Descricao);
   Auditoria.AdicionaCampoExcluido('cfop', Obj.CFOP);
   Auditoria.AdicionaCampoExcluido('suspensao_icms', Obj.suspensao_icms);
end;

procedure TRepositorioNaturezaOperacao.SetCamposIncluidos(
  Auditoria: TAuditoria; Objeto: TObject);
var
  Obj :TNaturezaOperacao;
begin
   ...
   Auditoria.AdicionaCampoIncluido('descricao',     Obj.Descricao);
   Auditoria.AdicionaCampoIncluido('cfop', Obj.CFOP);
   Auditoria.AdicionaCampoIncluido('suspensao_icms', Obj.suspensao_icms);   
end;
