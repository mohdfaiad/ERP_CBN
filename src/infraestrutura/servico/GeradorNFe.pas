...
begin
  ...
  self.AjustarConfiguracoesDaEmpresa(NotaFiscal.Empresa.ConfiguracoesNF);    
   ...
   with self.FACBrNFe.EventoNFe.Evento.Add do
    begin
      DhEvento    := IncSecond(TDateTimeUtilitario.DataSEFAZToDateTime(NotaFiscal.NFe.ObtemValorPorTag('dhRecbto')), 10);
...
//   self.GerarNFe(NotaFiscal);
   ...
   self.FACBrNFe.NotasFiscais.Items[0].NFe.procNFe.chNFe    := NotaFiscal.ChaveAcesso;
   self.FACBrNFe.NotasFiscais.Items[0].NFe.procNFe.dhRecbto := NotaFiscal.DataSaida;
   self.FACBrNFe.NotasFiscais.Items[0].NFe.procNFe.nProt    := self.FACBrNFe.WebServices.Consulta.Protocolo;
   self.FACBrNFe.NotasFiscais.Items[0].NFe.procNFe.digVal   := self.FACBrNFe.WebServices.Consulta.protNFe.digVal;
   self.FACBrNFe.NotasFiscais.Items[0].NFe.procNFe.cStat    := self.FACBrNFe.WebServices.Consulta.cStat;
...
begin
   ...
   try
     ...
     for nX := 0 to (EmailsDestinatario.Count-1) do begin
       if not(TStringUtilitario.EstaVazia(EmailsDestinatario[nX])) then
         CC.Add(EmailsDestinatario[nX]);
     end;

     if TStringUtilitario.EstaVazia(NotaFiscal.NFe.XMLText) then
      NotaFiscal.RecarregarNFe;

     ACBrMail1.Host                := NotaFiscal.Empresa.ConfiguracoesEmail.SMTPHost;
     ACBrMail1.Port                := NotaFiscal.Empresa.ConfiguracoesEmail.SMTPPort;
     ACBrMail1.Username            := NotaFiscal.Empresa.ConfiguracoesEmail.SMTPUser;
     ACBrMail1.Password            := NotaFiscal.Empresa.ConfiguracoesEmail.SMTPPassword;
     ACBrMail1.From                := NotaFiscal.Empresa.ConfiguracoesEmail.SMTPUser; { Remetente }
     ACBrMail1.SetSSL              := NotaFiscal.Empresa.ConfiguracoesEmail.UsaSSL; // SSL - Conex̣o Segura
...
begin
   ...
   try
     BuscaCidade         := TBuscaCidade.Create(nil);
     BuscaCidade.codCid  := IntToStr(Destinatario.Endereco.CodCidade);
     ...
     nfe.NFe.Dest.CNPJCPF           := Destinatario.CPF_CNPJ;
     nfe.NFe.Dest.xNome             := Destinatario.Razao;
     nfe.NFe.Dest.EnderDest.xLgr    := Destinatario.Endereco.Logradouro;
     nfe.NFe.Dest.EnderDest.cPais   := 1058;
     nfe.NFe.Dest.EnderDest.xPais   := Destinatario.Endereco.Pais;
     nfe.NFe.Dest.EnderDest.nro     := Destinatario.Endereco.Numero;
     nfe.NFe.Dest.EnderDest.xBairro := Destinatario.Endereco.Bairro;
     nfe.NFe.Dest.EnderDest.cMun    := BuscaCidade.CodigoMunicipio;
     nfe.NFe.Dest.EnderDest.xMun    := Trim(BuscaCidade.edtCidade.Text);
     nfe.NFe.Dest.EnderDest.UF      := Trim(BuscaCidade.edtUF.Text);
     nfe.NFe.Dest.EnderDest.CEP     := StrToInt(Destinatario.Endereco.CEP);
     nfe.NFe.Dest.EnderDest.fone    := TStringUtilitario.ApenasNumeros(Destinatario.Fone1);
   finally
     FreeAndNil(BuscaCidade);
   end;
end;
...
begin
   ...
   try
      BuscaCidade         := TBuscaCidade.Create(nil);
      BuscaCidade.codCid  := IntToStr(Emitente.Endereco.CodCidade);

      nfe.NFe.Emit.CNPJCPF           := Emitente.CPF_CNPJ;
      nfe.NFe.Emit.xNome             := Emitente.Razao;
      nfe.NFe.Emit.xFant             := Emitente.Razao;
      nfe.NFe.Emit.EnderEmit.xLgr    := Emitente.Endereco.Logradouro;
      nfe.NFe.Emit.EnderEmit.nro     := Emitente.Endereco.Numero;
      nfe.NFe.Emit.EnderEmit.xBairro := Emitente.Endereco.Bairro;
      nfe.NFe.Emit.EnderEmit.cMun    := BuscaCidade.CodigoMunicipio;
      nfe.NFe.Ide.cMunFG             := BuscaCidade.CodigoMunicipio;
      nfe.NFe.Emit.EnderEmit.xMun    := Trim(BuscaCidade.edtCidade.Text);
      nfe.NFe.Emit.EnderEmit.UF      := Trim(BuscaCidade.edtUF.Text);
      nfe.NFe.Emit.EnderEmit.CEP     := StrToInt(TStringUtilitario.ApenasNumeros(Emitente.Endereco.CEP));
      nfe.NFe.Emit.EnderEmit.fone    := TStringUtilitario.ApenasNumeros(Emitente.Fone1);
      nfe.NFe.Emit.IE                := Emitente.RG_IE;
      nfe.NFe.Emit.CRT               := crtRegimeNormal;

      { Para o caso do emitente ser empresa }
        case NF.Empresa.RegimeTributario of
...
begin
  ...
  nfe.NFe.Ide.natOp   := NF.Natureza.Descricao;
  ...
  if (NF.Natureza.Tipo in [tnoSaidaDentroEstado, tnoSaidaForaEstado, tnoExportacao]) then
  ...
  nfe.NFe.Ide.verProc  := '2.0'; //versao do erp
end;
...
begin
   ...
   try
      BuscaCidade         := TBuscaCidade.Create(nil);
      BuscaCidade.codCid  := IntToStr(NF.Transportadora.Endereco.CodCidade);
      ...
      nfe.NFe.Transp.Transporta.xMun    := Trim(BuscaCidade.edtCidade.Text);
      nfe.NFe.Transp.Transporta.UF      := Trim(BuscaCidade.edtUF.Text);
   finally
     FreeAndNil(BuscaCidade);
   end;
end;
...
//  nfe.NFe.Total.ICMSTot.vII     := self.GetTotalCampo( VL_II,         nfe );
...
//   Volume.nVol  := FloatToStr(nota.Numero);
end;
